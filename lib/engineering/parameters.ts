...
export const PARAMETERS: ParameterDefinition[] = [
  ...
  },
  {
    ...
    description:
      "Travel after hammer contact to the latch's critical point of no return. This is the high-resistance release window, not the hammer's full post-contact travel.",
    unit: "in",
    category: "requirement",
    min: 0.01,
    max: 0.5,
    ...
    dependencies: [],
    whatIfIncrease:
      "The spring must do work through a longer critical window and reaches that point with less residual force F3.",
  },
  {
    id: "y_total",
    symbol: canonicalSym("y_total"),
    name: canonicalName("y_total"),
    description:
      "Total post-contact travel while the hammer remains engaged with the latch, measured from first contact to the final coupled-travel stop.",
    unit: "in",
    category: "requirement",
    ...
    max: 1,
    step: 0.005,
    formula: "L4 = L2 + y_total",
    whatIfIncrease:
      "The spring must remain loaded over more travel, so end force F4 and the end-force margin both decrease.",
  },
  {
    id: "F_end_min",
    symbol: canonicalSym("F_end_min"),
    name: canonicalName("F_end_min"),
    description:
      "Minimum nominal gross spring force required at the end of full coupled travel. This is a design floor, not a predicted impact force.",
    unit: "lbf",
    category: "requirement",
    source: "Mechanism boundary",
    min: 0,
    max: 200,
    step: 0.5,
    formula: "F4 ≥ F_end_min",
    whatIfIncrease:
      "More end-force reserve is required, which removes candidates that relax too close to free length.",
  },
  {
    id: "F_opposing",
    symbol: canonicalSym("F_opposing"),
    name: canonicalName("F_opposing"),
    description:
      "Modeled opposing preload acting after the critical release point while the hammer and latch finish their coupled travel.",
    unit: "lbf",
    category: "requirement",
    source: "Mechanism assumption — verify",
    min: 0,
    max: 100,
    step: 0.1,
    formula: "F4,net = F4 − F_opposing",
    whatIfIncrease:
      "Net finishing force and net post-critical work decrease by the same added resistance.",
  },
  {
    id: "L_armed_min",
    symbol: "Larmed,min",
    name: "Optional Minimum Armed Spring Height",
    description:
      "Optional lower bound for the optimizer-derived compressed spring length. Leave unset when the mechanism has no separate armed-height window.",
    unit: "in",
    category: "requirement",
    source: "Optional mechanism boundary",
    min: 0.1,
    max: 3,
    step: 0.005,
  },
  {
    id: "L_armed_max",
    symbol: "Larmed,max",
    name: "Optional Maximum Armed Spring Height",
    description:
      "Optional upper bound for the optimizer-derived compressed spring length. This filters designs; it does not prescribe a supplier spring's loaded height.",
    unit: "in",
    category: "requirement",
    source: "Optional mechanism boundary",
    min: 0.1,
  ...
  {
    id: "W_critical",
    symbol: canonicalSym("W_critical"),
    name: canonicalName("W_critical"),
    description: "Spring work delivered from hammer contact through the critical release window.",
    unit: "in·lbf",
    category: "hammer",
    formula: "W_critical = F2·y_latch − ½·k·y_latch²",
    dependencies: ["F2", "k", "y_latch"],
    whatIfIncrease: "More spring work is available through the point-of-no-return window.",
  },
  {
    id: "W_post",
    symbol: canonicalSym("W_post"),
    name: canonicalName("W_post"),
    description: "Gross spring work delivered after critical release through the end of coupled travel.",
    unit: "in·lbf",
    category: "hammer",
    formula: "W_post = W_coupled − W_critical",
    dependencies: ["W_coupled", "W_critical"],
    whatIfIncrease: "More gross spring work is available to finish the mechanism travel.",
  },
  {
    id: "W_opposing",
    symbol: canonicalSym("W_opposing"),
    name: canonicalName("W_opposing"),
    description: "Work required to overcome the modeled constant opposing preload over the post-critical distance.",
    unit: "in·lbf",
    category: "hammer",
    formula: "W_opposing = F_opposing·y_post",
    dependencies: ["F_opposing", "y_post"],
    whatIfIncrease: "More of the post-critical spring work is consumed by the opposing preload.",
  },
  {
    id: "W_post_net",
    symbol: canonicalSym("W_post_net"),
    name: canonicalName("W_post_net"),
    description: "Post-critical spring work remaining after subtracting work against the modeled opposing preload.",
    unit: "in·lbf",
    category: "hammer",
    formula: "W_post_net = W_post − W_opposing",
    dependencies: ["W_post", "W_opposing"],
    whatIfIncrease: "More net quasi-static work is available to complete the remaining travel.",
  },
  {
    id: "W_coupled",
    symbol: canonicalSym("W_coupled"),
    name: canonicalName("W_coupled"),
    description: "Gross spring work delivered over the complete post-contact coupled travel.",
    unit: "in·lbf",
    category: "hammer",
    formula: "W_coupled = F2·y_total − ½·k·y_total²",
    dependencies: ["F2", "k", "y_total"],
    whatIfIncrease: "More spring work is available after contact through the final stop.",
  },
  {
    id: "KE",
    symbol: "KE",
  ...
  },
  {
    id: "F3",
    symbol: "F3",
    name: canonicalName("F3"),
    description:
      "Residual spring force at the critical point of no return, after the high-resistance release window.",
    unit: "lbf",
    category: "spring-state",
    formula: "F3 = F2 − k·y_latch",
    dependencies: ["F2", "k", "y_latch"],
    whatIfIncrease: "More force remains at the critical release point.",
    min: 0,
    max: 100,
    step: 0.1,
  },
  {
    id: "F4",
    symbol: canonicalSym("F4"),
    name: canonicalName("F4"),
    description:
      "Gross spring force remaining at the end of full coupled hammer/latch travel. This is compared with the configured end-force floor.",
    unit: "lbf",
    category: "spring-state",
    formula: "F4 = F2 − k·y_total",
    dependencies: ["F2", "k", "y_total"],
    whatIfIncrease: "More finishing-force reserve remains at the final mechanism stop.",
    min: 0,
    max: 100,
    step: 0.1,
  },
  {
    id: "F4_net",
    symbol: canonicalSym("F4_net"),
    name: canonicalName("F4_net"),
    description: "Net final driving force after subtracting the modeled opposing latch preload.",
    unit: "lbf",
    category: "spring-state",
    formula: "F4_net = F4 − F_opposing",
    dependencies: ["F4", "F_opposing"],
    whatIfIncrease: "More positive force is available to finish the remaining mechanism travel.",
  },
  {
    id: "k",
    symbol: "k",
  ...
  },
  {
    id: "L3",
    symbol: "L3",
    name: canonicalName("L3"),
    description:
      "Spring length at the critical point of no return: L3 = L2 + y_latch.",
    unit: "in",
    category: "spring-state",
    formula: "L3 = L2 + y_latch",
    dependencies: ["L2", "y_latch"],
    whatIfIncrease: "The spring is more relaxed at critical release, so residual force F3 is lower.",
  },
  {
    id: "L4",
    symbol: canonicalSym("L4"),
    name: canonicalName("L4"),
    description: "Spring length at the end of full coupled hammer/latch travel: L4 = L2 + y_total.",
    unit: "in",
    category: "spring-state",
    formula: "L4 = L2 + y_total",
    dependencies: ["L2", "y_total"],
    whatIfIncrease: "The spring is more relaxed at the final stop, reducing end force F4.",
  },
  {
    id: "y_post",
    symbol: canonicalSym("y_post"),
    name: canonicalName("y_post"),
    description: "Remaining coupled hammer/latch travel after the critical point of no return.",
    unit: "in",
    category: "spring-state",
    formula: "y_post = y_total − y_latch",
    dependencies: ["y_total", "y_latch"],
    whatIfIncrease: "The spring must provide positive residual drive over a longer finishing segment.",
  },
