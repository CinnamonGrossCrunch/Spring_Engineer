    ...
    <>
      <div className="flex flex-col rounded-lg border border-zinc-200 bg-white">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-zinc-200 px-3 py-2">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded bg-gradient-to-br from-violet-500 to-blue-600 px-3 py-2 text-[11px] font-semibold text-white shadow-sm hover:shadow-md hover:scale-105 transition-transform focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600"
              title="Generate CAD B-rep STEP geometry for this candidate"
            >
        ...
        <Group title="Work">
          ...
          <Row
            label={`${canonicalName("WreleaseIdeal")} ${canonicalSym("WreleaseIdeal")}`}
            value={fmtWork(c.WreleaseIdeal)}
            hint="Ideal upper bound — not energy delivered to the latch"
          />
          <Row
            label="Stress τ / TS"
          ...
          <div className="flex flex-col gap-2 px-3 pb-3">
            ...
            {(etaMode === "assumed" || etaMode === "measured") && (
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  min={0}
                  max={1}
                  onChange={(e) => setEtaValue(Number.parseFloat(e.currentTarget.value) || 0)}
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                step={0.01}
                min={0}
                onChange={(e) => setMass(Number.parseFloat(e.currentTarget.value) || 0)}
                className="w-[70px] rounded border border-zinc-300 px-1.5 py-1 text-right font-mono text-[12px] focus:border-blue-500 focus:outline-none"
              />
      ...
      {/* CAD Generation Modal */}
      <CadGenerationModal candidate={c} isOpen={cadModalOpen} onClose={() => setCadModalOpen(false)} />
    </>
  );
}
