    ...
    <section className="border-b border-zinc-200 bg-gradient-to-br from-zinc-950 via-zinc-900 to-blue-950 px-5 py-5 text-white">
      <div className="grid items-center gap-5 lg:grid-cols-[1.15fr_1fr]">
        <div>
          ...
          <p className="mt-1 text-xs text-zinc-300">Three operating states, one glance. Nominal model output—not vendor validation or measured impact force.</p>
          <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
            {[
              ["Hammer travel", `${c.s.toFixed(3)} in`],
              ["Hammer work", `${c.Whammer.toFixed(2)} in·lbf`],
              ["Contact force", `${c.F2.toFixed(1)} lbf`],
              ["Avg equivalent", lens.massAdjustedAverageEquivalent === undefined ? "masses required" : `${lens.massAdjustedAverageEquivalent.toFixed(0)} lbf`],
          ...
          {lens.massAdjustedAverageEquivalent === undefined && <p className="mt-2 rounded border border-amber-400/30 bg-amber-400/10 px-2 py-1.5 text-[10px] text-amber-200">Hammer and latch masses are incomplete, so this sheet shows spring work and ideal force equivalents only.</p>}
        </div>
        <div className="rounded-xl border border-white/10 bg-white p-3 text-zinc-900 shadow-xl">
          <svg viewBox="0 0 430 220" className="h-auto w-full" role="img" aria-label="Spring at armed, hammer-contact and follow-through lengths">
            <Spring x={80} length={c.Lc / base} label="ARMED" force={c.F0} color="#2563eb" />
            <Spring x={215} length={c.L2 / base} label="CONTACT" force={c.F2} color="#059669" />
            <Spring x={350} length={c.L3 / base} label="FOLLOW-THROUGH" force={c.F3} color="#c2410c" />
          </svg>
          <div className="grid grid-cols-3 gap-1 text-center text-[9px] text-zinc-500">
            <span>L {c.Lc.toFixed(3)} in</span><span>L {c.L2.toFixed(3)} in</span><span>L {c.L3.toFixed(3)} in</span>
          </div>
        </div>
      </div>
    </section>
